...
static int mxs_saif_write_ctrl_reg(u32 scr0, u32 scr1) {
	...
	__raw_writel((scr1 & ~(BM_SAIF_CTRL_SFTRST|BM_SAIF_CTRL_CLKGATE)) | BM_SAIF_CTRL_RUN, SAIF1_CTRL);
	mdelay(1);
	__raw_writel((scr0 & ~(BM_SAIF_CTRL_SFTRST|BM_SAIF_CTRL_CLKGATE)) | BM_SAIF_CTRL_RUN, SAIF0_CTRL);
				  ...
				  int div_id, int div)
{
	...
	if (saif_select->saif_clk == SAIF0)
		mxs_saif_write_ctrl_reg(scr, __raw_readl(SAIF1_CTRL));
	else
		mxs_saif_write_ctrl_reg(__raw_readl(SAIF0_CTRL), scr);
...
static int mxs_saif_set_dai_fmt(struct snd_soc_dai *cpu_dai, unsigned int fmt)
{
	...
	case SND_SOC_DAIFMT_CBM_CFM:
		if (saif_select->saif_clk == SAIF0) {
			mxs_saif_write_ctrl_reg((scr & ~BM_SAIF_CTRL_SLAVE_MODE) | scr0,
                                    (scr | BM_SAIF_CTRL_SLAVE_MODE) | scr1);
		} else {
			mxs_saif_write_ctrl_reg((scr | BM_SAIF_CTRL_SLAVE_MODE) | scr0,
                                    (scr & !BM_SAIF_CTRL_SLAVE_MODE) | scr1);
		}
...
static void reset_mxs_dma(void)
{
...
// Reset SAIF1
        ...
        while(!(HW_SAIF_CTRL_RD(1) & BM_SAIF_CTRL_CLKGATE));
