...
#if 0		//modefy by zhuguojun 下面代友造成串口在双工数据处理时，数据丢失
		//add by qianjianen 2013-01-10 
		if (plat->dir_set) {
			while(!((__raw_readl(s->port.membase + HW_UARTAPP_STAT) &
				 ...
				 struct ktermios *old)
{
	...
	if (cflag & CRTSCTS)
		ctrl2 |= BM_UARTAPP_CTRL2_CTSEN | BM_UARTAPP_CTRL2_RTSEN;    //modify by chenhaiman
	else
...
static void mxs_auart_shutdown(struct uart_port *u)
{
	...
	mydbg("\n");
	/*modify by chenhaiman, 此处软复位会清空还没发送的数据，
	    用以下代码可实现把剩余数据发送完再关闭串口*/
	//__raw_writel(BM_UARTAPP_CTRL0_SFTRST,
