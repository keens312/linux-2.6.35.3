...
#define CONFIG_PROCESS_DURATION 27
#define COLOR_NUM 5
#define CONFIG_BOOT_PROCESS_BAR
#define CONFIG_BOOT_PROCESS

static struct delayed_work boot_process_work;
static u32 palette_24[COLOR_NUM] = {200, 203, 206, 209, 212};
static u32 palette_32[COLOR_NUM] = {52, 92, 132, 172, 212};
...
static void draw_box(int x, int y, int width, int height, int color_index)
{
    ...
    switch (fb->var.bits_per_pixel) {
        case 24:
            palette = (void *)palette_24;
            break;
        case 32:
            palette = (void *)palette_32;
            break;
        default:
            palette = (void *)palette_24;
            break;
...
static void process_worker(struct work_struct * work)
{
    ...
    if (timestamp == 0) {
        ...
        process_width   = block_width * PROCESS_BLOCK_NUM;
        ...
        for (i = 0 ; i < 4; i++) {
            ...
            if (current_x > (process_x + process_width - block_width) || current_x < process_x) {
                continue; 
            }
