...
#if defined(CONFIG_EPC_283B) || defined(CONFIG_EPC_287B) || defined(CONFIG_EPC_280) || defined(CONFIG_A287_WB128LI) || defined(CONFIG_A287_W128LI) 
	{
	 .name = "AUART2.TX",
	 .id   = PINID_SSP2_SCK,
	 .fun  = PIN_FUN2,
...
#endif
#if defined(CONFIG_EPC_28xA) || defined(CONFIG_IoT_3960) || defined(CONFIG_IoT_3962)
	{
	 .name = "AUART2.TX",
	 .id   = PINID_AUART2_TX,
	 .fun  = PIN_FUN1,
...
#if defined(CONFIG_A287_WB128LI) || defined(CONFIG_A287_W128LI)
	{ 
	 .name  = "AUART3.CTS",
	 .id    = PINID_AUART3_CTS,
	 .fun   = PIN_FUN1,	
...
#if 	defined(CONFIG_EPC_28xA) \
	|| 	defined(CONFIG_EPC_287B) \
	||	defined(CONFIG_IoT_3960) \
	||	defined(CONFIG_IoT_3962) \
	||	defined(CONFIG_A287_WB128LI) \
	||	defined(CONFIG_A287_W128LI) 
#if !defined(CONFIG_IoT_3960) && !defined(CONFIG_IoT_3962)
	{
	 .name	= "CAN1_TX",
	 .id	= PINID_GPMI_CE2N,
	 .fun	= PIN_FUN2,
...
//280 wifi pwr pins
#if defined(CONFIG_EPC_280) && (defined(CONFIG_BCMDHD_WEXT) || defined(CONFIG_BCMDHD_WEXT_MODULE))
	{
     .name  = "GPIO-3_26",
...
#endif  //end 280 wifi pwr pins
#endif  //end mmc0

	//SDIO2 PINS
#if defined(CONFIG_SDIO2PDEV)
	//D1 D2
#if defined(CONFIG_A287_WB128LI) || defined(CONFIG_A287_W128LI) || defined(CONFIG_IoT_3962)
    {
     .name  = "SSP2_DATA1",
     .id    = PINID_SSP2_SS1,
     .fun   = PIN_FUN2,
...
#elif defined(CONFIG_EPC_28xA) || defined(CONFIG_EPC_287B) || defined(CONFIG_IoT_3960)
    {
     .name  = "SSP2_DATA1",
     .id    = PINID_SSP1_SCK,
     .fun   = PIN_FUN2,
...
#elif defined(CONFIG_A287_WB128LI) || defined(CONFIG_A287_W128LI) || defined(CONFIG_IoT_3962)
	{
     .name  = "GPIO-3_25",
     .id    = PINID_I2C0_SDA,
     .fun   = PIN_GPIO,
...
#elif defined(CONFIG_EPC_28xA) || defined(CONFIG_IoT_3960)
    {
     .name  = "GPIO-3_26",
     .id    = PINID_SAIF1_SDATA0,
     .fun   = PIN_GPIO,
...
#if defined(CONFIG_EASY283_PWM) || defined(CONFIG_EASY283_PWM_MODULE)
#if defined(CONFIG_EPC_280)
	{
	 .name	= "PWM_3",
	 .id	= PINID_PWM3,  //3_28
...
// static struct pin_desc mx28evk_sdio2_pins[] = {

// };
// void mx28evk_sdio2_pins_init(void)
// {
	// printk("sdio wifi pins init.\n");
	// mx28evk_init_pin_group(mx28evk_sdio2_pins,
				  // ARRAY_SIZE(mx28evk_sdio2_pins));
// }
// EXPORT_SYMBOL_GPL(mx28evk_sdio2_pins_init);

static  struct pin_desc mx28evk_auido_pins[] = {
#if defined(CONFIG_SND_MXS_SOC) || defined(CONFIG_SND_MXS_SOC_MODULE)
	// Configurations of SAIF0 port pins 
	{
	 ...
	 .fun	= PIN_FUN1,
	 //.strength	= PAD_12MA,
	 .strength	= PAD_4MA,					/* modefy by patch fsl		*/
	 .voltage	= PAD_3_3V,
	 //.pullup	= 1,
	 .pullup	= 0,						/* modefy by patch fsl		*/	
	 .drive 	= 1,
	 .pull 		= 1,
	 },
	{
...
void mx28evk_auido_pins_init(void)
{
	printk("sgtl-5000 pins init \n");
	mx28evk_init_pin_group(mx28evk_auido_pins,
				  ARRAY_SIZE(mx28evk_auido_pins));
}
EXPORT_SYMBOL_GPL(mx28evk_auido_pins_init);
...
// static struct pin_desc mx28evk_spi2_pins[] = {

// };

// void mx28evk_spi2_pins_init(void)
// {
	// printk("spi2 pins init \n");
	// mx28evk_init_pin_group(mx28evk_spi2_pins,
				  // ARRAY_SIZE(mx28evk_spi2_pins));
// }
// EXPORT_SYMBOL_GPL(mx28evk_spi2_pins_init);
// #endif
...
#if defined(CONFIG_EPC_28xA) || defined(CONFIG_EPC_280) 
	{
	 .name	= "SSP2 MOSI",
	 .id	= PINID_SSP2_MOSI,
	 .fun	= PIN_FUN1,
...
#elif defined(CONFIG_EPC_283B) || defined(CONFIG_EPC_287B)
	{
	 .name	= "SSP2 MOSI",
     .id    = PINID_SSP0_DATA6,
	 .fun	= PIN_FUN2,
...
#if defined(CONFIG_EPC_280)
#define	 PHYRESETPIN    PINID_LCD_RS
#else    //others
#define	 PHYRESETPIN    PINID_LCD_D16
#endif

#if defined(CONFIG_EPC_28xA)
#define	 PHY1RESETPIN    PINID_SPDIF
#endif
...
int mx28evk_enet_gpio_init(void)
{
	...
	gpio_request(MXS_PIN_TO_GPIO(PHYRESETPIN), "PHY_RESET");
	gpio_direction_output(MXS_PIN_TO_GPIO(PHYRESETPIN), 0);
	gpio_request(MXS_PIN_TO_GPIO(PHY1RESETPIN), "PHY1_RESET");
	gpio_direction_output(MXS_PIN_TO_GPIO(PHY1RESETPIN), 0);
	...
	mdelay(50);
	gpio_direction_output(MXS_PIN_TO_GPIO(PHYRESETPIN), 1);
	gpio_direction_output(MXS_PIN_TO_GPIO(PHY1RESETPIN), 1);

	return 0;
#endif    //end CONFIG_EPC_28xA
}
