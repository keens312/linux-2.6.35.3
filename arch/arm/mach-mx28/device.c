...
#if	defined(CONFIG_I2C_MXS_SELECT0) && !defined(CONFIG_A287_WB128LI) && !defined(CONFIG_A287_WB128LI)
...
{
//EPC has no poweron and wp  pin in ssp0 
	int ret = 0;
	return ret;
}
...
static void __init mx28_init_wdt(void)
{
    ...
    printk("mx28 wdt: mxs_add_device.\r\n");
...
static struct sp706s_platform_data sp706s_pdata = {
     .gpio = PINID_GPMI_RDY1,
     .expires = HZ / 2 ,
};                                                                                                                                  
 
static struct platform_device sp706s_device = {
    .name = "sp706s",
    .id   = -1,
    .dev  = {
        .platform_data  = &sp706s_pdata,
    },
};

static void __init sp706s_wdt(void)
{
    platform_device_register(&sp706s_device);

    printk("sp706 wdt: platform device registered.\r\n");
}
...
static void __init mx28_init_wdt(void)
{
    ...
    printk("sp706 wdt: do'nt register platfor device.\r\n");
	;
}
static void __init sp706s_wdt(void)
{
...
void __init mx28_init_fec(void)
{
	struct platform_device *pdev;
	struct mxs_dev_lookup *lookup;
...
#if defined(CONFIG_FEC_L2SWITCH)  || defined(CONFIG_FEC_L2SWITCH_MODULE)
...
void __init mx28_init_l2switch(void)
{
	struct platform_device *pdev;
...
static void __init mx28_init_ts(void)
{
	...
	&& !defined(CONFIG_EPC_280) && !defined(CONFIG_EPC_283B)
...
{
	struct clk *pll_clk;
	struct clk *clk1;						/* add by patch fsl		*/
	...
	clk_enable(clk);
	clk_set_rate(clk1, 12000000);					/* patch by fsl			*/
	clk_enable(clk1);						/* patch by fsl			*/
...
static void __init mx28_init_spdif(void)
{	struct platform_device *pdev;
...
#if !defined(CONFIG_FEC_MODULE)
    mx28_init_fec();
#endif
#if !defined(CONFIG_FEC_L2SWITCH_MODULE)
    mx28_init_l2switch();
#endif
...
static int __init imx28_ocotp_init2(void)
{
    return 0;
}
module_init(imx28_ocotp_init2);

static void __exit imx28_ocotp_exit2(void)
{
    return;
}
module_exit(imx28_ocotp_exit2);

EXPORT_SYMBOL(mx28_init_fec);
EXPORT_SYMBOL(mx28_init_l2switch);
