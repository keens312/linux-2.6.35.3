...
int mxs_del_devices(struct platform_device *pdev, int num, int level)
{
	int i, ret = -ENOMEM;
	if (pdev == NULL || IS_ERR(pdev) || num <= 0)
		return -EINVAL;
	...
	if ((mxs_device_num - num) < 0)
		goto out;
	mxs_device_num -= num;
	for (i = 0; i < num; i++)
		list_del(&pdev[i].dev.devres_head);
	ret = 0;
out:
	mutex_unlock(&device_mutex);
	return ret;
...
int mxs_del_device(struct platform_device *pdev, int level)
{
	return mxs_del_devices(pdev, 1, level);
}
...
#if defined(CONFIG_I2C_MXS_SELECT0) && !defined(CONFIG_A287_WB128LI) && !defined(CONFIG_A287_W128LI)
	{
	 .name	= "mxs-i2c",
	 .id	= 0,
	 .dev = {
...
static struct platform_device mxs_fec[] = {
    ...
    defined(CONFIG_FEC_L2SWITCH_MODULE)
